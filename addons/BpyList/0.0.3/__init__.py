...
class VIEW3D_PT_BpyList(Panel):
    ...
    def is_editing_target(context):
        ...
        gp_edit = obj and obj.mode in {
            'EDIT_GPENCIL', 'PAINT_GPENCIL', 'SCULPT_GPENCIL', 'WEIGHT_GPENCIL'}
    ...
    def draw(self, context):
        ...
        location = 'F:\\projects\\BlendExec\\bpy_scripts'
        # r=>root, d=>directories, f=>files
        i = 0
        col_ue = layout.column()
        box_ue = layout.column().box()
        # row_ue = col_ue.box().row()
        # row_ue.label(text='ue')
            ...
            for item in f:
                if '.py' in item:
                    ...
                    if '[UE]' in filename_1:
                        ...
                        row = box.row()
                    op = row.operator('bpylist.run', text="",
                                      icon='RADIOBUT_ON')
    ...
    @staticmethod
    def menu_remove():
        if not hasattr(VIEW3D_PT_BpyList, '_menu_original'):
            return
